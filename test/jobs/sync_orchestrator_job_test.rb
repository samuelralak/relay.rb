...
class SyncOrchestratorJobTest < ActiveSupport::TestCase
  ...
  test "job is enqueued to sync queue" do
    assert_equal "sync", SyncOrchestratorJob.new.queue_name
  end
  ...
  test "executes without error with no relays configured" do
    assert_nothing_raised do
      SyncOrchestratorJob.new.perform
    end
  end
  ...
  test "executes with realtime mode by default" do
    ...
    assert_nothing_raised do
      SyncOrchestratorJob.new.perform(mode: "realtime")
    end
  end
  ...
  test "executes with upload mode" do
    assert_nothing_raised do
      SyncOrchestratorJob.new.perform(mode: "upload")
    end
  end
  ...
  test "can be enqueued with perform_later" do
    assert_enqueued_with(job: SyncOrchestratorJob, queue: "sync") do
      SyncOrchestratorJob.perform_later(mode: "realtime")
    end
  end

  test "enqueued job preserves mode argument" do
    SyncOrchestratorJob.perform_later(mode: "backfill")

    enqueued_job = enqueued_jobs.find { |j| j["job_class"] == "SyncOrchestratorJob" }
