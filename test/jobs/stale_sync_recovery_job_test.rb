...
class StaleSyncRecoveryJobTest < ActiveSupport::TestCase
  ...
  test "job is enqueued to sync queue" do
    assert_equal "sync", StaleSyncRecoveryJob.new.queue_name
  end
  ...
  test "can be enqueued with perform_later" do
    assert_enqueued_with(job: StaleSyncRecoveryJob, queue: "sync") do
      StaleSyncRecoveryJob.perform_later
    end
  end
end
